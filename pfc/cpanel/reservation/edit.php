...
if (isset($_GET['id_client']) && isset($_GET['id_chambre']) && isset($_GET['date_reservation']) ) { 
...
$sql = "UPDATE `reservation` 
		SET  `id_chambre` =  '{$_POST['id_chambre']}' ,  `date_reservation` =  '{$_POST['date_reservation']}' ,  `date_arrive` =  '{$_POST['date_arrive']}' ,  `date_depart` =  '{$_POST['date_depart']}'   
...
$row = mysql_fetch_array ( mysql_query("SELECT * FROM `reservation` 
												 WHERE `id_client` = $id_client 
												 AND `id_chambre` = $id_chambre 
												 AND 'date_reservation'='$date_reservation' ")); 
?>
